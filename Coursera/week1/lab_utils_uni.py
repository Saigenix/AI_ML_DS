...
# Embedded file name: /home/jovyan/work/lab_utils_uni.py
...
n_bin = 5
dlcm = LinearSegmentedColormap.from_list('dl_map',
  dlcolors, N=n_bin)
...
def mk_cost_lines(x, y, w, b, ax):
    ...
    for p in zip(x, y):
        ...
        ax.annotate(f"{c_p_txt:0.0f}", xy=cxy, xycoords='data', color=dlpurple, xytext=(5,
                                                                                        0),
          textcoords='offset points')
        cstr += f"{c_p_txt:0.0f} +"
        if len(cstr) > 38:
            if addedbreak is False:
                cstr += '\n'
                addedbreak = True
            ctot += c_p
...
def plt_stationary(x_train, y_train):
    ...
    w_range = np.array([-100.0, 500])
    b_range = np.array([-250.0, 350])
    ...
    ax[1].text(0.5, 0.95, 'Click to choose w,b', bbox=dict(facecolor='white', ec='black'), fontsize=10, transform=(ax[1].transAxes),
...
class plt_update_onclick:
    ...
    def __call__(self, event):
        if event.inaxes == self.ax[1]:
            ...
            d = self.ax[1].annotate(f"Cost: {cst:.0f}", xy=(ws, bs), xytext=(4, 4), textcoords='offset points', bbox=dict(facecolor='white'),
              size=10)
...
def inbounds(a, b, xlim, ylim):
    ...
    if ax > xlow:
        if ax < xhigh:
            pass
        return bx > xlow and bx < xhigh and ay > ylow and ay < yhigh and by > ylow and by < yhigh and True
    else:
        return False


def plt_contour_wgrad(x, y, hist, ax, w_range=[-100, 500, 5], b_range=[-500, 500, 5], contours=[
 0.1, 50, 1000, 5000, 10000, 25000, 50000], resolution=5, w_final=200, b_final=100, step=10):
    ...
    CS = ax.contour(w0, b0, z, contours, linewidths=2, colors=[
     dlblue, dlorange, dldarkred, dlmagenta, dlpurple])
...
def add_line(dj_dx, x1, y1, d, ax):
    ...
    ax.annotate(('$\\frac{\\partial J}{\\partial w}$ =%d' % dj_dx), fontsize=14, xy=(
     x1, y1),
      xycoords='data',
      xytext=(
     xoff, 10),
...
def plt_gradients(x_train, y_train, f_compute_cost, f_compute_gradient):
    ...
    ax[1].quiverkey(Q, 0.9, 0.9, 2, '$2 \\frac{m}{s}$', labelpos='E', coordinates='figure')
    ax[1].set_xlabel('w')
    ax[1].set_ylabel('b')